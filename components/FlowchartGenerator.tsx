...
import { generateFlowchart } from "../lib/apila";
...
export default function FlowchartGenerator() {
  ...
  const handleGenerate = async () => {
    if (!subject.trim() || !process.trim()) {
      Alert.alert("Missing Fields", "Please fill in subject and process");
      return;
    }
    ...
    try {
      const response = await generateFlowchart(subject, process, language);
      setResult(response);
    } catch (error: any) {
      console.error(error);
      Alert.alert("Error", error.message || "Failed to generate flowchart");
    } finally {
      setLoading(false);
    }
  };
    ...
    <View className="space-y-4">
      ...
      <View>
        ...
        <View className="flex-row items-center bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-2xl px-4 py-4 focus:border-purple-500">
          ...
          <TextInput
            className="flex-1 ml-3 text-gray-900 dark:text-white"
            placeholder="e.g. Photosynthesis, Digestion"
            placeholderTextColor="#9CA3AF"
            value={process}
