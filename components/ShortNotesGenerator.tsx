...
export default function ShortNotesGenerator() {
  ...
  const generateNotes = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        error.message || "Failed to generate notes. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View className="space-y-4">
      ...
      <View>
        ...
        <View className="flex-row bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-xl px-2 py-2">
          {["Sinhala", "English"].map((lang) => (
            <TouchableOpacity
              key={lang}
              onPress={() => setLanguage(lang)}
              className={`flex-1 py-2 items-center rounded-lg ${
                language === lang ? "bg-white dark:bg-gray-700" : ""
              }`}
            >
              <Text
                className={`${
        ...
        {loading ? (
          ...
          <View className="flex-row items-center">
            <Ionicons
              ...
              style={{ marginRight: 8 }}
            />
            <Text className="text-white font-bold text-lg">Generate Notes</Text>
          </View>
        )}
      </TouchableOpacity>
